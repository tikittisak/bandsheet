# Bandsheet v6.03 Design Language Manual

เอกสารนี้เป็น source of truth สำหรับออกแบบและสร้าง app ใหม่ในแนวทางเดียวกับ Bandsheet v6.03 โดยยึด `_template.html` v6.03 เป็น baseline ที่ใช้งานจริง ไม่ใช่การออกแบบจากศูนย์
...
Bandsheet v6.03 คือ working surface สำหรับนักดนตรีและคนจัด chord chart ไม่ใช่ landing page หรือ document viewer ธรรมดา เป้าหมายคือทำให้ rehearsal อ่านเร็ว แก้เร็ว และ export ได้ทันที
...
โครงสร้างหน้า v6.03 แบ่งเป็น 4 ชั้น:
...
Tokens ต่อไปนี้มาจาก `_template.html` v6.03 และควรเป็น baseline ของ project ใหม่
...
- `Inter`: toolbar, metabar, labels, bar numbers, export header
- `Lato`: default chord font
- `DM Sans`, `Petaluma`, `Inter`, `Lato`: selectable chord fonts
...
- Header ใช้ Inter, title 24px, artist 14px, meta 13px
...
<main class="bandsheet-static">
  <div class="toolbar">
    ...
    <div class="brand">bandsheet by ti.muse <span>v6.03</span></div>
  </div>
...
<style>
  ...
  .bar-cell .num {
    ...
    z-index: 1;
    color: var(--bar-label);
    font: 600 8px/8px Inter, sans-serif;
  }
...
ใช้ checklist นี้เมื่อสร้าง project ใหม่จากหลักการ v6.03:
...
- อย่าเริ่มจากหน้าเปล่า ให้เริ่มจาก v6.03 behavior ที่พิสูจน์แล้ว: toolbar, metabar, mini sheet, edit mode, export

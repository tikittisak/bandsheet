# Bandsheet v6.17 Design Language Manual

เอกสารนี้เป็น local source of truth สำหรับ Bandsheet โดยยึด `_template.html` v6.17 เป็น baseline ที่ใช้งานจริง ไม่ใช่การออกแบบจากศูนย์

ก่อนใช้ไฟล์นี้กับงาน UI ให้อ่าน shared design layer ที่ root ของ Vault ก่อน:

- `../DESIGN_PHILOSOPHY.md` — philosophy รวมของทุก project
- `../design/UI_PATTERNS.md` — pattern ที่ใช้ซ้ำ
- `../design/TOKENS.md` — token baseline กลาง
- `../design/COMPONENT_RULES.md` — component/control rules กลาง
- `../design/LESSONS_LEARNED.md` — lesson ข้าม project

ไฟล์นี้เก็บเฉพาะ local law ของ Bandsheet: chord chart surface, section colors, rhythm strip, export sheet, data contract, import guard, และ HTML-to-Figma snippets

## File Decision

ใช้ `design-language.md` เป็นไฟล์หลัก เพราะเหมาะกับการทำงานของโปรเจกต์นี้ที่สุด:
...
Bandsheet v6.17 คือ working surface สำหรับนักดนตรีและคนจัด chord chart ไม่ใช่ landing page หรือ document viewer ธรรมดา เป้าหมายคือทำให้ rehearsal อ่านเร็ว แก้เร็ว และ export ได้ทันที
...
โครงสร้างหน้า v6.17 แบ่งเป็น 4 ชั้น:
...
Tokens ต่อไปนี้มาจาก `_template.html` v6.17 และควรเป็น baseline ของ project ใหม่
...
<main class="bandsheet-static">
  <div class="toolbar">
    ...
    <div class="brand">bandsheet by ti.muse <span>v6.17</span></div>
  </div>
...
ใช้ checklist นี้เมื่อสร้าง project ใหม่จากหลักการ v6.17:
...
- อย่าเริ่มจากหน้าเปล่า ให้เริ่มจาก v6.17 behavior ที่พิสูจน์แล้ว: toolbar, metabar, mini sheet, edit mode, export
